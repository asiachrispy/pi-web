...
**关联**：[总计划](./plan-pi-web-macos-workbench.md) · [M1 设计](./m1-design.md)
...
2. **默认用户 vs 高级** — 场景首页、简洁工具、模型配置（ModelsConfig）为主路径；会话树、完整工具、远程、命令面板等走「高级」入口。
...
4. pi-web 产品态偏好：`~/.pi/agent/pi-web-preferences.json`（onboarding、工具模式、`defaultWorkspaceCwd` 等），不替代 `auth.json` / `settings.json`。
...
| 里程碑 | 清单 |
|--------|------|
| M1 | [m1-checklist.md](./m1-checklist.md) · [m1-design.md](./m1-design.md) |
| M2 | [m2-checklist.md](./m2-checklist.md) |
| M3 | [m3-checklist.md](./m3-checklist.md) |
| M4 | [m4-checklist.md](./m4-checklist.md) |
