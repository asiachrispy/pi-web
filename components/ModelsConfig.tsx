...
function ModelDetail({
  ...
  const testSummary = useMemo(() => {
    ...
    if (testState.phase === "testing") return t("modelsConfig.testingModelConnection");
    const meta = [
      testState.latencyMs !== undefined ? `${testState.latencyMs}ms` : null,
      testState.status !== undefined ? `HTTP ${testState.status}` : null,
    ].filter(Boolean);
    if (testState.phase === "success") {
      return [t("modelsConfig.connected"), ...meta, testState.responseText || null].filter(Boolean).join(" · ");
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <SectionTitle>{t("modelsConfig.thinkingLevelMap")}</SectionTitle>
      ...
      <div>
        <SectionTitle>{t("modelsConfig.costPerMillionTokens")}</SectionTitle>
  ...
  const submitCode = useCallback(async (token: string, code: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setLoginState({ phase: "error", message: d.error ?? t("modelsConfig.serverError", { status: res.status }) });
        return;
      }
      setInputValue("");
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: provider.loggedIn ? "#4ade80" : "var(--text-dim)" }}>
            {provider.loggedIn ? t("modelsConfig.connectedState") : t("modelsConfig.notConnectedState")}
          </span>
        </div>
      </div>
      ...
      <div style={{ minHeight: 48 }}>
        {loginState.phase === "idle" && (
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
            {provider.loggedIn ? t("modelsConfig.alreadyConnected") : t("modelsConfig.connectAccount", { provider: provider.name })}
          </p>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
              {loginState.phase === "auth"
                ? t("modelsConfig.browserCompleteHint")
                : loginState.message}
            </p>
            {loginState.phase === "auth" && (
              <p style={{ margin: 0, fontSize: 11, color: "var(--text-dim)", lineHeight: 1.5 }}>
                {t("modelsConfig.openLoginPageHint")}{" "}
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <input
                ...
                placeholder={loginState.phase === "auth" ? t("modelsConfig.callbackPlaceholder") : (loginState.placeholder ?? t("modelsConfig.enterValuePlaceholder"))}
        ...
        {loginState.phase === "device_code" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
              {t("modelsConfig.verificationPageHint")}
            </p>
            ...
            <p style={{ margin: 0, fontSize: 11, color: "var(--text-dim)", lineHeight: 1.5 }}>
              ...
              {loginState.expiresInSeconds ? ` ${t("modelsConfig.expiresInMinutes", { count: Math.ceil(loginState.expiresInSeconds / 60) })}` : ""}
            </p>
          </div>
        )}
        ...
        {loginState.phase === "success" && (
          <p style={{ margin: 0, fontSize: 12, color: "#4ade80" }}>{t("modelsConfig.connectedSuccessfully")}</p>
        )}
...
function ApiKeyDetail({ provider, onRefresh }: { provider: ApiKeyProvider; onRefresh: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <SectionTitle>{t("modelsConfig.apiKeyShort")}</SectionTitle>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: provider.configured ? "#4ade80" : "var(--text-dim)" }}>
            {provider.configured ? t("modelsConfig.configured") : t("modelsConfig.notConnectedState")}
          </span>
        </div>
      </div>
      ...
      <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {provider.configured
          ? t("modelsConfig.apiKeyStored")
          : t("modelsConfig.enterApiKey", { provider: provider.displayName, count: provider.modelCount })}
      </p>

      <Field label={t("modelsConfig.apiKeyShort")}>
        <div style={{ display: "flex", gap: 6 }}>
          <SecretTextInput
            value={apiKey}
            onChange={setApiKey}
            onKeyDown={(e) => { if (e.key === "Enter" && apiKey.trim()) handleSave(); }}
            placeholder={provider.configured ? t("modelsConfig.enterNewKeyPlaceholder") : t("modelsConfig.apiKeyMaskedPlaceholder")}
            style={{ flex: 1 }}
            autoComplete="off"
            spellCheck={false}
            mono
          ...
          >
            ...
            {savedOk ? t("modelsConfig.saved") : saving ? t("modelsConfig.saving") : t("common.save")}
          </button>
        </div>
      </Field>
      ...
      {provider.configured && (
        ...
        >
          {removing ? t("modelsConfig.removing") : t("modelsConfig.disconnect")}
        </button>
      )}
    </div>
  );
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          {totalCount === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(240px, 100%), 1fr))", gap: 8 }}>
              {showCustom && (
                <div style={{ gridColumn: "1 / -1", fontSize: 10, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("modelsConfig.customGroup")}</div>
              )}
              {showCustom && (
                <button
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t("modelsConfig.openAiCompatible")}</div>
              ...
              {availableOAuth.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{t("modelsConfig.oauth")}</div>
                  </div>
              ...
              {availableApiKey.length > 0 && (
                <div style={{ gridColumn: "1 / -1", paddingTop: availableOAuth.length > 0 ? 6 : 0, fontSize: 10, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("modelsConfig.apiKeyShort")}</div>
              )}
              {availableApiKey.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{t("modelsConfig.modelsCount", { count: p.modelCount })}</div>
                  </div>
                  <ProviderIcon id={p.id} size={28} />
                </button>
              ))}
      ...
      <div style={{ width: 860, height: "78vh", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.18)", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>{t("modelsConfig.models")}</span>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
          ...
          <div style={{ width: 210, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", flexShrink: 0, background: "var(--bg-panel)" }}>
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 6px" }}>
              ...
              {/* Custom providers */}
              {loading ? (
                <div style={{ padding: "10px 8px", fontSize: 12, color: "var(--text-muted)" }}>{t("common.loading")}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, padding: "10px 18px", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <button onClick={handleSave} disabled={saving || savedOk} style={{
            ...
            <span>{savedOk ? t("modelsConfig.saved") : saving ? t("modelsConfig.saving") : t("common.save")}</span>
          </button>
        </div>
      </div>
    </div>
